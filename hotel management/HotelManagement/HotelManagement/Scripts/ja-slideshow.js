...
JaSlideshowClass = new Class({
	...
	start: function() {
		...
		this.image.setStyles({display: 'block', position: 'absolute', left: 'auto', right: '0', top: '0', zIndex: 1});
	...
	preload: function(fast) {
		...
		if (this.loader.complete && ((new Date).getTime() > this.timer[0])) {
			...
			if (fast) {
				img.setStyles({right: '0px', opacity: 1, top: '0px'});
			...
			if (this.options.type.test(/push|wipe/)) {
				...
				img.setStyles({left: 'auto', right: '0px'});
				...
				img.setStyles({left: 'auto', top: 'auto', right: '0px', bottom: 'auto'});
